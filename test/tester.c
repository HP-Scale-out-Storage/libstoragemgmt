...
START_TEST(test_disk_sg_path)
{
    ...
    if( LSM_ERR_OK == rc ) {
        ...
        for( i = 0; i < count; ++i ) {
            G(rc, lsm_disk_sg_path_get, d[i], &sg_path);

            if (LSM_ERR_OK == rc)
                printf("SCSI generic path: (%s)\n", sg_path);

            rc = lsm_disk_sg_path_get(d[i], NULL);
            fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);

        }

        rc = lsm_disk_sg_path_get(NULL, &sg_path);
        fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);
...
START_TEST(test_disk_sas_address)
{
    uint32_t count = 0;
    lsm_disk **d = NULL;
    ...
    if( LSM_ERR_OK == rc ) {
        ...
        for( i = 0; i < count; ++i ) {
            G(rc, lsm_disk_sas_address_get, d[i], &sas_address);

            if (LSM_ERR_OK == rc)
                printf("Disk sas_address: (%s)\n", sas_address);

            rc = lsm_disk_sas_address_get(d[i], NULL);
            fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);

        }

        rc = lsm_disk_sas_address_get(NULL, &sas_address);
        fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);
...
START_TEST(test_disk_sep_sg_path)
{
    uint32_t count = 0;
    lsm_disk **d = NULL;
    const char *sep_sg_path = NULL;
    int i = 0;
    ...
    if( LSM_ERR_OK == rc ) {
        ...
        for( i = 0; i < count; ++i ) {
            G(rc, lsm_disk_sep_sg_path_get, d[i], &sep_sg_path);

            if (LSM_ERR_OK == rc)
                printf("SEP SCSI generic path: (%s)\n", sep_sg_path);

            rc = lsm_disk_sep_sg_path_get(d[i], NULL);
            fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);

        }

        rc = lsm_disk_sep_sg_path_get(NULL, &sep_sg_path);
        fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);
...
START_TEST(test_disk_clear_ident_led)
{
    int rc;
    ...
    G(rc, lsm_disk_clear_ident_led, c, disks[0], LSM_CLIENT_FLAG_RSVD);

    G(rc, lsm_disk_record_array_free, disks, disk_count);
}
END_TEST

START_TEST(test_disk_set_fault_led)
{
    int rc;
    ...
    G(rc, lsm_disk_set_fault_led, c, disks[0], LSM_CLIENT_FLAG_RSVD);

    G(rc, lsm_disk_record_array_free, disks, disk_count);
}
END_TEST

START_TEST(test_disk_clear_fault_led)
{
    int rc;
    ...
    G(rc, lsm_disk_clear_fault_led, c, disks[0], LSM_CLIENT_FLAG_RSVD);
...
START_TEST(test_volume_sd_path)
{
    lsm_volume *volume = NULL;
    char *job = NULL;
    const char *sd_path = NULL;
    ...
    G(rc, lsm_volume_sd_path_get, volume, &sd_path);

    if (LSM_ERR_OK == rc && sd_path != NULL)
        printf("SCSI device path: (%s)\n", sd_path);

    rc = lsm_volume_sd_path_get(volume, NULL);
    fail_unless(LSM_ERR_INVALID_ARGUMENT == rc, "rc = %d", rc);
...
Suite * lsm_suite(void)
{
    ...
    tcase_add_test(basic, test_disk_clear_ident_led);
    tcase_add_test(basic, test_disk_set_fault_led);
    tcase_add_test(basic, test_disk_clear_fault_led);
