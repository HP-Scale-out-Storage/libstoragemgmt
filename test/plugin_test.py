...
class TestPlugin(unittest.TestCase):
    ...
    def test_disk_sas_address_get(self):
        for s in self.systems:
            cap = self.c.capabilities(s)
            if supported(cap, [Cap.DISK_SAS_ADDR]):
                for disk in self.c.disks():
                    disk_sas_address = disk.disk_sas_address
                    self.assertTrue(disk_sas_address is not None and
    ...
    def test_disk_sep_sg_path_get(self):
        for s in self.systems:
            cap = self.c.capabilities(s)
            if supported(cap, [Cap.DISK_SEP_SG_PATH]):
                for disk in self.c.disks():
                    disk_sep_sg_path = disk.disk_sep_sg_path
                    self.assertTrue(disk_sep_sg_path is not None and
                                    len(disk_sep_sg_path) > 0,
    ...
    def test_disk_clear_ident_led(self):
        for s in self.systems:
            cap = self.c.capabilities(s)
            if supported(cap, [Cap.DISK_LED]):
                for disk in self.c.disks():
                    disk_led_status = self.c.disk_clear_ident_led(disk)
                    self.assertTrue(disk_led_status is None,
                                    "Disk clear_ident_led"
                                    "failed")

    def test_disk_set_fault_led(self):
        for s in self.systems:
            cap = self.c.capabilities(s)
            if supported(cap, [Cap.DISK_LED]):
                for disk in self.c.disks():
                    disk_led_status = self.c.disk_set_fault_led(disk)
                    self.assertTrue(disk_led_status is None,
                                    "Disk set_fault_led"
                                    "failed")

    def test_disk_clear_fault_led(self):
        for s in self.systems:
            cap = self.c.capabilities(s)
            if supported(cap, [Cap.DISK_LED]):
                for disk in self.c.disks():
                    disk_led_status = self.c.disk_clear_fault_led(disk)
                    self.assertTrue(disk_led_status is None,
                                    "Disk clear_fault_led"
                                    "failed")
    ...
    def test_volume_sg_path_get(self):
        for s in self.systems:
            cap = self.c.capabilities(s)
            if supported(cap, [Cap.VOLUME_SG_PATH]):
                for volume in self.c.volumes():
                    vol_sg_path = volume.vol_sg_path
                    self.assertTrue(vol_sg_path is not None and
                                    len(vol_sg_path) > 0,
                                    "Volume scsi device node retrieval failed")
