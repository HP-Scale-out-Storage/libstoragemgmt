...
def disk_clear_ident_led_test(cap):
    if cap['DISK_LED']:
        out = call([cmd, '-t' + sep, 'list', '--type', 'disks'])[1]
        disk_list = parse(out)
        for disk in disk_list:
            out = call([
                cmd, '-t' + sep, 'disk-clear-ident-led', '--disk', disk[0]])[1]
...
def disk_clear_fault_led_test(cap):
    if cap['DISK_LED']:
        out = call([cmd, '-t' + sep, 'list', '--type', 'disks'])[1]
        disk_list = parse(out)
        for disk in disk_list:
            out = call([
                cmd, '-t' + sep, 'disk-clear-fault-led', '--disk', disk[0]])[1]
