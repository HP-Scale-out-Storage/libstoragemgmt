...
class Client(INetworkAttachedStorage):
    ...
    ## Clear the IDENT LED for a physical disk.
    # @param    self            The this pointer
    # @param    disk            Disk object to target
    # @param    flags           Flags
    # @returns  lsmError on errors
    @_return_requires(unicode)
    def disk_clear_ident_led(self, disk, flags=FLAG_RSVD):
        """
        lsm.Client.disk_clear_ident_led(self, disk, flags=lsm.Client.FLAG_RSVD)

        Version:
            1.3
        Usage:
            This method disables the IDENT LED on a physical disk if supported
        Parameters:
            disk (lsm.Disk)
                An lsm.Disk object.
            flags (int)
        ...
        SpecialExceptions:
        """
        return self._tp.rpc('disk_clear_ident_led', _del_self(locals()))

    ## Set the FAULT LED for a physical disk.
    # @param    self            The this pointer
    # @param    disk            Disk object to target
    # @param    flags           Flags
    # @returns  lsmError on errors
    @_return_requires(unicode)
    def disk_set_fault_led(self, disk, flags=FLAG_RSVD):
        """
        lsm.Client.disk_set_fault_led(self, disk, flags=lsm.Client.FLAG_RSVD)

        Version:
            1.3
        Usage:
            This method enables the FAULT LED on a physical disk if supported
        Parameters:
            disk (lsm.Disk)
                An lsm.Disk object.
            flags (int)
