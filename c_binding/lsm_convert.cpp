...
lsm_volume *value_to_volume(Value & vol)
{
    ...
    if (is_expected_object(vol, CLASS_NAME_VOLUME)) {
        ...
        if ((rc != NULL) && (_STD_MAP_HAS_KEY(v, "vol_sg_path") == 1) &&
            (v["vol_sg_path"].asC_str()[0] != '\0')) {
            
            if (lsm_volume_sg_path_set(rc, v["vol_sg_path"].asC_str()) !=
                LSM_ERR_OK) {
                lsm_volume_record_free(rc);
                rc = NULL;
                throw ValueException("value_to_volume: failed to update "
...
Value volume_to_value(lsm_volume * vol)
{
    if (LSM_IS_VOL(vol)) {
        ...
        if (vol->vol_sg_path != NULL)
            v["vol_sg_path"] = Value(vol->vol_sg_path);
        return Value(v);
    }
    return Value();
}
...
lsm_disk *value_to_disk(Value & disk)
{
    ...
    if (is_expected_object(disk, CLASS_NAME_DISK)) {
        ...
        if ((rc != NULL) && (_STD_MAP_HAS_KEY(d, "disk_sas_address") == 1) &&
            (d["disk_sas_address"].asC_str()[0] != '\0')) {
            
            if (lsm_disk_sas_address_set(rc, d["disk_sas_address"].asC_str()) !=
                LSM_ERR_OK) {
                lsm_disk_record_free(rc);
                rc = NULL;
                throw ValueException("value_to_disk: failed to update "
        ...
        if ((rc != NULL) &&
            (_STD_MAP_HAS_KEY(d, "disk_sep_sas_address") == 1) &&
            (d["disk_sep_sas_address"].asC_str()[0] != '\0')) {
            
            if (lsm_disk_sep_sas_address_set(rc,
                d["disk_sep_sas_address"].asC_str()) !=
                LSM_ERR_OK) {
                lsm_disk_record_free(rc);
                rc = NULL;
                throw ValueException("value_to_disk: failed to update "
