...
static int handle_disk_set_ident_led(lsm_plugin_ptr p, Value & params,
                                     Value & response)
{
    int rc = LSM_ERR_NO_SUPPORT;
    if (p && p->ops_v1_3 && p->ops_v1_3->disk_set_ident) {
        ...
        if (Value::object_t == v_disk.valueType() &&
            ...
            rc = p->ops_v1_3->disk_set_ident(p, disk,
                                             LSM_FLAG_GET_VALUE(params));
...
static int handle_disk_clear_ident_led(lsm_plugin_ptr p, Value & params,
                                     Value & response)
{
    int rc = LSM_ERR_NO_SUPPORT;
    if (p && p->ops_v1_3 && p->ops_v1_3->disk_clear_ident) {
        Value v_disk = params["disk"];
        ...
        if (Value::object_t == v_disk.valueType() &&
            ...
            rc = p->ops_v1_3->disk_clear_ident(p, disk,
                                               LSM_FLAG_GET_VALUE(params));
...
static int handle_disk_set_fault_led(lsm_plugin_ptr p, Value & params,
                                     Value & response)
{
    int rc = LSM_ERR_NO_SUPPORT;
    if (p && p->ops_v1_3 && p->ops_v1_3->disk_set_fault) {
        Value v_disk = params["disk"];
        ...
        if (Value::object_t == v_disk.valueType() &&
            ...
            rc = p->ops_v1_3->disk_set_fault(p, disk,
                                             LSM_FLAG_GET_VALUE(params));
