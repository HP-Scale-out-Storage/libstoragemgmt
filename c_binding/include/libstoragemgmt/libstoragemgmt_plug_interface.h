...
/**
 * Enable the IDENT LED for the desired disk.
 ...
 */
typedef int (*lsm_plug_disk_set_ident_led) (lsm_plugin_ptr c,
                                            lsm_disk * disk,
                                            lsm_flag flags);
 ...
 * @return LSM_ERR_OK on success else error reason.
 */
typedef int (*lsm_plug_disk_set_fault_led) (lsm_plugin_ptr c,
                                            lsm_disk * disk,
                                            lsm_flag flags);

/**
 * Disable the FAULT LED for the desired disk.
 * New in version 1.3, only available for hardware RAID and HBA cards.
 * @param[in] c			Valid lsm plug-in pointer
 * @param[in] disk		A single lsm_disk
 * @param[in] flags         	Reserved, set to 0
 * @return LSM_ERR_OK on success else error reason.
 */
typedef int (*lsm_plug_disk_clear_fault_led) (lsm_plugin_ptr c,
                                              lsm_disk * disk,
                                              lsm_flag flags);
...
struct lsm_ops_v1_3 {
    ...
    lsm_plug_disk_set_fault_led disk_set_fault;
    lsm_plug_disk_clear_fault_led disk_clear_fault;
};
...
/**
 * New in version 1.3. Set a disk's scsi generic path.
 * @param disk		Pointer to the disk of interest.
 * @param sg_path	Pointer to the disk's scsi generic path.
 * @return LSM_ERR_OK on success, else error reason.
 */
int LSM_DLL_EXPORT lsm_disk_sg_path_set(lsm_disk *disk,
                                              const char *sg_path);
 ...
 * @return LSM_ERR_OK on success, else error reason.
 */
int LSM_DLL_EXPORT lsm_disk_sas_address_set(lsm_disk *disk,
                                              const char *sas_addr);
